...
public class ImageUtils {
	 ...
	 * @throws IOException
	 */
	public final static void scale(File sourceImg, File output, int scale, boolean flag) throws IOException {
	 ...
	 */
	public final static void scale(File sourceImg, File output, int height, int width, boolean flag) {
		try {
	 ...
	 *            比例不对时是否需要补白：true为补白; false为不补白;
	 */
	public final static byte[] scale(byte[] img, int height, int width, boolean flag) {
		try {
			double ratio = 0.0; // 缩放比例
	 ...
	 *            目标切片高度
	 */
	public final static void cut(File sourceImg, File output, int x, int y, int width, int height) {
		try {
			// 读取源图像
			BufferedImage bi = ImageIO.read(sourceImg);
